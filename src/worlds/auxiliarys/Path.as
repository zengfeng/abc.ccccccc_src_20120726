package worlds.auxiliarys {
	...
	public class Path
	{
		...
		// ---------------------------------- 我是优美的长分隔线 ---------------------------------- //
		private var id : int;
		private static var arithmetic : *;
		private static var setPathSizeFun : Function;
		private static var writePathDataFun : Function;
		private static var findPathFun : Function;
		private static var setBlockFun : Function;
		...
		private function decodePathFile(ba : ByteArray) : void
		{
			width = ba.readInt() ;
			height = ba.readInt() ;
			mapWidth = MapUtil.pathToMapX(width);
			mapHeight = MapUtil.pathToMapY(height);
...
//			setBarrier(255, true);
		...
		public function setBarrier(barrierId : uint, isOpened : Boolean) : void
		{
			...
			setBlockFun(id, barrierId, isOpened ? 1 : 0);
		}
		...
		private function writeData() : void
		{
			...
			setPathSizeFun(id, width, height);
			line = 0;
			lineNum = 128 ;
			startWriteData();
		}
		...
		private function writeDataLine(line : int, lineNum : int) : void
		{
			...
			writePathDataFun(id, line, lineNum, alchemyByteArray);
			alchemyByteArray.clear();
		}
		...
		{
			if(mapFromX < 0)  mapFromX = 0;
			if(mapFromY < 0)  mapFromY = 0;
			if(mapToX < 0)  mapToX = 0;
			if(mapToY < 0)  mapToY = 0;
			
			if(mapFromX >mapWidth) mapFromX = mapWidth;
			if(mapFromY >mapHeight) mapFromY = mapHeight;
			if(mapToX >mapWidth) mapToX = mapWidth;
			if(mapToY >mapHeight) mapToY = mapHeight;
			...
			findPathFun(id, pathFromPoint.x, pathFromPoint.y, pathToPoint.x, pathToPoint.y, findByteArray);

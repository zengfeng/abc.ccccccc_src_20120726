package worlds.roles.animations.depths
{
	...
	public class RoleNode
	{
		...
		protected var tempParent : DisplayObjectContainer;
		...
		private function addToLayerHander() : void
		{
			...
			tempIndex = linkList.sortAdd(this) + 2;
			avatar.x = -300;
			avatar.y = -300;
		...
		private function updateDepthHander() : void
		{
			...
			tempParent.addChildAt(avatar, tempIndex);
			if (pre)
			{
				tempIndex = tempParent.getChildIndex(pre.avatar) + 1;
				if (tempParent.getChildIndex(avatar) != tempIndex)
				{
					tempParent.addChildAt(avatar, tempIndex);
				}
			}

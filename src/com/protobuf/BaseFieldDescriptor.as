...
package com.protobuf
{
	...
	public class BaseFieldDescriptor {
		...
		public static function getExtensionByName(name : String) : BaseFieldDescriptor {
			const fieldPosition : int = name.lastIndexOf('/');
			if (fieldPosition == -1) {
				return BaseFieldDescriptor(UIManager.appDomain.getDefinition(name));
			} else {
				return UIManager.appDomain.getDefinition(name.substring(0, fieldPosition))[name.substring(fieldPosition + 1)];
			}
		}

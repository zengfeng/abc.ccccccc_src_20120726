...
//TODO 分析战斗模式
			if (PLAYMODLE == 0) {
				...
				{
					var isDugeonBoss : Boolean = false;    //设置boss
					var vomster : VoMonster = RSSManager.getInstance().getMosterById(EnemyRoleID) as VoMonster; //获取怪物信息
					
					mapID = MapUtil.currentMapId;                                    //获取地图ID
					if (vomster && vomster.IsDugeonBoss())
						isDugeonBoss = true;                                         //如果是大Boss
					
					if (isDugeonBoss)
						mapID = mapID * 10000 + 1;                                   //获取Boss地图
					else
						mapID = mapID * 10000;                                       
					
					
					mapD = AttackData.mapidlist[mapID] as mapData;                  //TODO 获取地图数据和地图URL
					picNameStr = mapD.urlStr;                                       

					if (isDugeonBoss) // 大boss截图                                 //TODO 如果是大Boss
					{
						if (BackGroundPic != null) {                                 //TODO 对于没有背景的
				...
				else if (BTSystem.INSTANCE().getFightTypeID() == BTSystem.FT_ARTIFACTS)    //TODO 神器战斗
				{
					mapID = getCityID() + 2;
					mapD = AttackData.mapidlist[mapID] as mapData;
					picNameStr = mapD.urlStr;
...
//========================================================================				
			// 战斗层
			_container.addChild(_battleSprite);     //_container是总容器么
			_container.show();
...
//==================================================
//TODO:创建了一个敌人头像的Panel  该Panel是Gcomponent
//==================================================			
			if (!enemypanel)
...
//==================================================
//TODO: 创建了一个用户的头像  该Panel是Gcomponent
//==================================================			
			if (!userpanel)
...
//==================================================					
		...
		public function end() : void
		 {
			var i : int = 0;
			BTSystem.INSTANCE().isInBattle = false;
...
//==================================================
//TODO: _swfSpriteDown层资源和avatar移除
//==================================================
				// _swfSpriteDown

package game.net.data.StoC {
	/**
	 * Server to Client 协议号0x92
	 **/
	import com.protobuf.*;
	import flash.utils.IDataInput;
	import flash.errors.IOError;
	public dynamic final class SCGuildChat extends com.protobuf.Message {
		 /**
		  *@from   from
		  **/
		...
		override public final function readFromSlice(input:flash.utils.IDataInput, bytesAfterSlice:uint):void {
			...
			while (input.bytesAvailable > bytesAfterSlice) {
				...
				case 1:
					if (from$count != 0) {
						throw new flash.errors.IOError('Bad data format: SCGuildChat.from cannot be set twice.');
					}
					++from$count;
				...
				case 2:
					if (content$count != 0) {
						throw new flash.errors.IOError('Bad data format: SCGuildChat.content cannot be set twice.');
					}
					++content$count;
				...
				case 3:
					if (potential$count != 0) {
						throw new flash.errors.IOError('Bad data format: SCGuildChat.potential cannot be set twice.');
					}
					++potential$count;
				...
				case 4:
					if (server$count != 0) {
						throw new flash.errors.IOError('Bad data format: SCGuildChat.server cannot be set twice.');
					}
					++server$count;
